...
export async function GET(_request: Request, { params }: { params: { code: string } }) {
  ...
  const token =
    ...
    "";
  try {
    const resp = await apiFetch(`${BASE}/${encodeURIComponent(params.code)}`, {
      method: "GET",
...
export async function PUT(request: Request, { params }: { params: { code: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { code: 500, msg: e instanceof Error ? e.message : "更新存储配置失败" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(_request: Request, { params }: { params: { code: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { code: 500, msg: e instanceof Error ? e.message : "删除存储配置失败" },
      { status: 500 }
    );
  }
}
