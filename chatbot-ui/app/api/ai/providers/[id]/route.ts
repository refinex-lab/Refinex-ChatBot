...
const BASE = `${AI_SERVICE_BASE_URL}/ai/providers`;
...
export async function PUT(request: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { code: 500, msg: error instanceof Error ? error.message : "更新模型供应商失败" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(_: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { code: 500, msg: error instanceof Error ? error.message : "删除模型供应商失败" },
      { status: 500 }
    );
  }
}
